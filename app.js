...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================
  // 5. INTERACTIVE RSVP FORM HANDLER
  // ==========================================
  ...
  const loadStoredRsvp = () => {
    const storedResponse = localStorage.getItem('rsvp_response');
    if (storedResponse) {
      try {
        const data = JSON.parse(storedResponse);
        showSuccessState(data.name, data.attendance, data.guests);
      } catch (e) {
  ...
  const showSuccessState = (name, attendance, guests) => {
    rsvpForm.classList.add('hidden');
    rsvpSuccess.classList.remove('hidden');

    if (attendance === 'yes') {
      successMsg.innerHTML = `<strong>Thank you, ${name}!</strong><br>We have recorded that you will attend with <strong>${guests} guest(s)</strong>. We look forward to celebrating with you!`;
    } else {
      successMsg.innerHTML = `<strong>Thank you for letting us know, ${name}.</strong><br>We will miss you at the celebrations, but we appreciate your blessings from afar!`;
    }
  };
  ...
  if (rsvpForm) {
    rsvpForm.addEventListener('submit', (e) => {
      ...
      const wishesInput = document.getElementById('rsvp-wishes').value.trim();
      ...
      setTimeout(() => {
        ...
        // Save locally
        localStorage.setItem('rsvp_response', JSON.stringify(rsvpData));
        ...
        // Show Success Block
        showSuccessState(nameInput, attendanceInput, guestsInput);
  ...
  if (editRsvpBtn) {
    editRsvpBtn.addEventListener('click', () => {
      ...
      // Autofill fields from local storage
      const storedResponse = localStorage.getItem('rsvp_response');
      if (storedResponse) {
        const data = JSON.parse(storedResponse);
        document.getElementById('rsvp-name').value = data.name || '';
        document.getElementById('rsvp-attendance').value = data.attendance || '';
        document.getElementById('rsvp-guests').value = data.guests || '1';
        document.getElementById('rsvp-wishes').value = data.wishes || '';
      }
    });
  }

  // Load RSVP on startup
  loadStoredRsvp();
